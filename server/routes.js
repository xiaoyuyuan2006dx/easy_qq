    ...
    try {
      ...
      if (req.method === 'GET' && pathname === '/backend/image-proxy') {
        ...
        try {
          ...
          if (['127.0.0.1', '::1', 'localhost', '0.0.0.0'].includes(host) ||
            ...
            return json(res, 403, { error: 'internal urls are not allowed' });
          }
      ...
      if (req.method === 'GET' && pathname === '/backend/health') {
        ...
        return json(res, 200, {
          ...
          uiSettings: state.uiSettings || { bgImageUrl: '', bgOpacity: 20, bgPosX: 50, bgPosY: 50, selfMsgColor: '#ffe4a8' },
      ...
      if (req.method === 'GET' && pathname === '/backend/ui-settings') {
        const settings = state.uiSettings && typeof state.uiSettings === 'object'
          ? { bgImageUrl: String(state.uiSettings.bgImageUrl || ''), bgOpacity: Number(state.uiSettings.bgOpacity || 20), bgPosX: Number(state.uiSettings.bgPosX || 50), bgPosY: Number(state.uiSettings.bgPosY || 50), selfMsgColor: String(state.uiSettings.selfMsgColor || '#dbeafe'), chatTheme: String(state.uiSettings.chatTheme || '柔和') }
          : { bgImageUrl: '', bgOpacity: 20, bgPosX: 50, bgPosY: 50, selfMsgColor: '#ffe4a8', chatTheme: '柔和' };
        if (!settings.bgImageUrl) {
          const defaultBg = path.join(config.LOCAL_FILE_ROOT, 'default.png');
          if (fs.existsSync(defaultBg)) {
            settings.bgImageUrl = '/files/default.png';
          }
        }
      ...
      if (req.method === 'POST' && pathname === '/backend/files/copy-to-local') {
        ...
        const dirPathRaw = String((body && body.dirPath) || '').trim().replace(/^[\/\\]+/, '');
        const rootResolved = path.resolve(config.LOCAL_FILE_ROOT);
        const dirResolved = dirPathRaw ? path.resolve(rootResolved, dirPathRaw) : rootResolved;
      ...
      if (req.method === 'GET' && pathname === '/backend/files/local') {
        const urlObj = new URL(req.url, `http://${req.headers.host || `127.0.0.1:${config.PORT}`}`);
        const rawPath = String(urlObj.searchParams.get('path') || '').trim().replace(/^[\/\\]+/, '');
        const rootResolved = path.resolve(config.LOCAL_FILE_ROOT);
        const resolved = rawPath ? path.resolve(rootResolved, rawPath) : rootResolved;
      ...
      if (req.method === 'POST' && pathname === '/backend/files/local/mkdir') {
        const body = await readBody(req);
        const rawPath = String((body && body.path) || '').trim().replace(/^[\/\\]+/, '');
        const rootResolved = path.resolve(config.LOCAL_FILE_ROOT);
        if (!rawPath) return json(res, 400, { error: 'path required' });
        const resolved = path.resolve(rootResolved, rawPath);
        if (!resolved.startsWith(rootResolved + path.sep) && resolved !== rootResolved) return json(res, 403, { error: 'access denied: path outside root' });
        try {
      ...
      if (req.method === 'GET' && pathname === '/backend/files/local/download') {
        const urlObj = new URL(req.url, `http://${req.headers.host || `127.0.0.1:${config.PORT}`}`);
        const rawPath = String(urlObj.searchParams.get('path') || '').trim();
        if (!rawPath) return json(res, 400, { error: 'path required' });
      ...
      if (req.method === 'POST' && pathname === '/backend/files/local/rename') {
        const body = await readBody(req);
        const oldPath = String((body && body.path) || '').trim().replace(/^[\/\\]+/, '');
        ...
        const rootResolved = path.resolve(config.LOCAL_FILE_ROOT);
        const oldResolved = path.resolve(rootResolved, oldPath);
        if (!oldResolved.startsWith(rootResolved + path.sep) && oldResolved !== rootResolved)
          return json(res, 403, { error: 'access denied: path outside root' });
      ...
      if (req.method === 'POST' && pathname === '/backend/files/local/save') {
        const body = await readBody(req);
        const rawDir = String((body && body.dirPath) || '').trim().replace(/^[\/\\]+/, '');
        ...
        const rootResolved = path.resolve(config.LOCAL_FILE_ROOT);
        const dirResolved = rawDir ? path.resolve(rootResolved, rawDir) : rootResolved;
        if (!dirResolved.startsWith(rootResolved + path.sep) && dirResolved !== rootResolved)
          return json(res, 403, { error: 'access denied: path outside root' });
        try {
          if (!fs.existsSync(dirResolved)) fs.mkdirSync(dirResolved, { recursive: true });
