...
function touchDevice(req, pathname, accepted, viaWs = false) {
  ...
  store.saveDevicesDebounced(state.devices);  // debounced, not immediate
}
...
function appendAuditLog(level, action, req, detail = {}) {
  ...
  store.appendAudit(row);  // append-only write
}
...
function checkAccess(req, pathname, isUpgrade = false) {
  ...
  const host = normalizeHost(parsed.host);
  const port = parsed.port;
  const allowedPorts = [config.PORT, HTTPS_PORT];
  const isLocalFixed = host === '127.0.0.1' && allowedPorts.includes(port);
  if (isLocalFixed) {
