const state = {
  ...
  custom: { bgImageUrl: '', bgOpacity: 20, bgPosX: 50, bgPosY: 50, selfMsgColor: '#ffe4a8' },
  authed: false,
  loginFailures: 0,
  selfId: '',
  selfNickname: '',
...
function dedupeMessages(list) {
  ...
  for (const m of list) {
    ...
    const roundedTime = Math.floor(Number((m && m.time) || 0));
...
async function refreshHealth() {
  ...
  el('wsTip').textContent = 'NapCat 的 ws 客户端填写地址：ws://你的IP:18080/ws?access_token=你的wstoken（如需 wss 则端口为 18443）';
  ...
  if (mode === 'server') {
    ...
    if (hasFSApi()) {
      ...
      const hint = isSecureCtx()
        ...
        : `点击"选择目录"浏览本机文件夹（仅可读取。如需写入请用 https://${location.hostname}:18443 或 http://localhost:${location.port} 访问）`;
